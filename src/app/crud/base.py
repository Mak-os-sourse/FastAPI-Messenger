...
class BaseCRUD[T: Base]:
    ...
    async def get_all(self, session: AsyncSession, whereclause: list[BinaryExpression] = None, **equality_where) -> list[T]:
        data = self.get_where(**equality_where)
        stmt = select(self.model).where(*data, *whereclause if whereclause is not None else [])
        result = await session.scalars(stmt)
        return result.all()
    
    async def get_one(self, session: AsyncSession, whereclause: list[BinaryExpression] = None, **equality_where) -> T | None:
